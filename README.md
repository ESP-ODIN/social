...
- cmd/api: configuration, database, shared authenticator and HTTP server.
- internal/router: routes and authkit middleware wiring.
- internal/handler: health, readiness and post HTTP handlers.
- internal/dto and internal/model: request and persisted post.
- internal/service/implementation: post validation and construction.
- internal/repository/implementation: PostgreSQL persistence.
- internal/testutil: local authentication fixtures used by tests.
- config and db: environment configuration and database pool.
